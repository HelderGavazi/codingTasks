...
def edit_task(app):
    ...
    """
    view_all_tasks(app)
    index = int(input("Enter the serial number of the task you would like to edit: ")) - 1
...
def update_task_status(app):
    ...
    view_all_tasks(app)
    index = (
        int(input("Enter the serial number of the task you want to update the status for: ")) - 1
    )
    if is_valid_task_index(app, index):
        tasks = read_all_tasks(app)
        if tasks:
    ...
    try:
        with open(app.all_tasks_file, "r") as file:
            for line in file:
                ...
                if len(task_data) == 3:  # Ensure correct format: name, description, is_complete
                    tasks.append(Task(task_data[0], task_data[1], bool(int(task_data[2]))))
...
def assign_task(app):
    """Assign a task to an employee by email. Only admin can assign task."""
...
def delete_task(app):
    ...
    if not isinstance(app.current_user, AdminUser):
        ...
        return
    view_all_tasks(app)
    index = int(input("Enter the serial number of the task you would like to delete: ")) - 1
    if is_valid_task_index(app, index):
        tasks = read_all_tasks(app)
        if tasks:
